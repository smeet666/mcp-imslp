 ...
 *
 * IMSLP publishes `Crawl-delay: 2` in its robots.txt and is run by a small
 * organisation living on donations, so requests run one at a time with a floor
 * on the gap between starts. When the site does push back, the interval doubles
 * and then decays back down as requests succeed, which recovers faster than a
 * fixed delay and behaves better than a constant rate.
 */
