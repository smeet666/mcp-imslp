...
`mcp-imslp` is a read-only client for [IMSLP, the Petrucci Music Library](https://imslp.org). It runs on your
...
**One host is contacted: `imslp.org`.** Nothing else.

| Host        | What is read there                       |
| ----------- | ---------------------------------------- |
| `imslp.org` | the library's API and its rendered pages |
...
| A `User-Agent`         | `mcp-imslp/<version> (+https://github.com/smeet666/mcp-imslp)`, so the site can reach a person about the traffic it receives. |
| Your IP address        | Sent by your network to any host you contact, as with any web request.                                                        |

Your requests reach IMSLP, the Petrucci Music Library. What is done with them there is governed by that site's own privacy policy, which this project does not control.
...
becomes of them. `IMSLP_LOG_LEVEL` governs how much is written and defaults to `error`. These lines stay on your machine.

## The settings that change any of this

| Variable             | What it changes                                                                                                  |
| -------------------- | ---------------------------------------------------------------------------------------------------------------- |
| `IMSLP_USER_AGENT`   | Adds your own identifier in front of this project's, which stays appended so the site can always reach a person. |
| `IMSLP_CACHE_TTL_MS` | How long an answer is held in memory. `0` turns the cache off.                                                   |
| `IMSLP_LOG_LEVEL`    | How much is written to stderr.                                                                                   |
...
Open an issue on [the repository](https://github.com/smeet666/mcp-imslp/issues). For something exploitable,
...
`mcp-imslp` est un client en lecture seule pour [IMSLP, la Petrucci Music Library](https://imslp.org). Il
...
**Un seul hôte est joint : `imslp.org`.** Rien d'autre.

| Hôte        | Ce qui y est lu                               |
| ----------- | --------------------------------------------- |
| `imslp.org` | l'API de la bibliothèque et ses pages rendues |
...
| Un `User-Agent`   | `mcp-imslp/<version> (+https://github.com/smeet666/mcp-imslp)`, pour que le site puisse joindre une personne au sujet du trafic qu'il reçoit. |
| Votre adresse IP  | Transmise par votre réseau à tout hôte que vous joignez, comme pour n'importe quelle requête web.                                             |

Vos requêtes atteignent IMSLP, la Petrucci Music Library. Ce qui en est fait là-bas relève de la politique de confidentialité de ce site, que ce projet ne contrôle pas.
...
qu'ils deviennent. `IMSLP_LOG_LEVEL` règle leur quantité et vaut `error` par défaut. Ces lignes restent sur votre machine.

## Les réglages qui changent tout cela

| Variable             | Ce qu'elle change                                                                                                        |
| -------------------- | ------------------------------------------------------------------------------------------------------------------------ |
| `IMSLP_USER_AGENT`   | Ajoute votre identifiant devant celui du projet, qui reste accolé pour que le site puisse toujours joindre une personne. |
| `IMSLP_CACHE_TTL_MS` | Combien de temps une réponse est gardée en mémoire. `0` éteint le cache.                                                 |
| `IMSLP_LOG_LEVEL`    | La quantité écrite sur stderr.                                                                                           |
...
Ouvrez une issue sur [le dépôt](https://github.com/smeet666/mcp-imslp/issues). Pour quelque chose
