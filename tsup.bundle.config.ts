 ...
 *
 * A bundle is installed by unpacking it rather than by resolving a package, so
 * nothing can stay external: the dependencies are compiled into the one file
 * that ships. The npm build keeps them external, so a consumer's own copies win
